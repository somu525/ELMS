...
if (isset($_POST['add_department'])) {
    ...
    echo "<div class='alert alert-success mt-3'>Department added successfully.</div>";
...
if (isset($_POST['delete_department'])) {
    $id = $_POST['delete_id'];
    $benchId=24;
    $conn->query("UPDATE Employees SET department_id = $benchId WHERE department_id = $id");
    $stmt = $conn->prepare("DELETE FROM Departments WHERE department_id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    echo "<div class='alert alert-danger mt-3'>Department deleted successfully.Employees are moved to bench</div>";
    echo "<script>setTimeout(() => window.location.href='departments.php', 1000);</script>";
}
